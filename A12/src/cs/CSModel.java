 ...
 * Method List: main
 * Constants List:
 * Purpose: Staring point of the CA program
 * 
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            String[] assignments = {"[A12]CA – Cellular Automata",
                    "[A22]GL – Game of Life", "[A32]TM – Turing Machine",
                    "[A32] TM Client"};
            ...
            okButton.addActionListener(e -> {
                ...
                switch (Objects.requireNonNull(selectedItem)) {
                    case "[A12]CA – Cellular Automata" -> {
                        SwingUtilities.invokeLater(CA::new);
                        dialog.setVisible(false);
                    }
                    case "[A22]GL – Game of Life" -> JOptionPane.showMessageDialog(dialog, selectedItem);
                    case "[A32]TM – Turing Machine" -> JOptionPane.showMessageDialog(dialog, selectedItem);
                    case "[A32] TM Client" -> JOptionPane.showMessageDialog(dialog, selectedItem);
                }
            });
